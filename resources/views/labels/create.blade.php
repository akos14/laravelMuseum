    ...
    <div class="container mx-auto p-3 lg:px-36">
        <div class="grid grid-cols-3 gap-6 px-32">
            <div class="col-span-3 lg:col-span-3 p-2 border rounded-lg bg-gray-100">
                <h2 class="font-semibold text-3xl my-2 inline-block p-3">Új címke felvitele</h2>
                <a href="{{ route('home') }}"
                ...
                <p class="mb-2 p-3">Ezen az oldalon lehet új címkét felvinni a rendszerbe. A tárgyakat úgy lehet
                    hozzárendelni, ha a címke létrehozása után módosítod a tárgyat, és ott bejelölöd ezt a címkét is.
                </p>
                <div class="ml-5 w-[96%] border-t border-gray-400"></div>
                <form x-data="{ labelName: '{{ old('name', '') }}', bgColor: '{{ old('bg-color', '#ffffffff') }}', display: '{{ old('display') }}' }" x-init="() => {
                    new Picker({
                        color: bgColor,
                        popup: 'bottom',
